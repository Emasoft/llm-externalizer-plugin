 ...
 * every retry and (b) gets written into report files (the "Unavailable
 * models" section, ~line 4444) — noisy AND a privacy leak of the account id
 * into a file the user may share. This keeps only the human-readable message +
 * the upstream provider detail, capped, and scrubs any id / key tokens.
 *
 * SAFETY: callers prepend `API error <status> (<backend>): ` themselves, so the
 * status code that classifyError() regex-matches (`/API error 429\b/` etc.) is
 * OUTSIDE what this returns and is always preserved — this only cleans the
 * response-body portion. Kept as a pure exported function so it is unit-tested
 * offline with zero spend (provider-error-sanitize.test.ts).
 */
...
const mcpServer = new McpServer(
  { name: "llm-externalizer", version: "10.1.0" },
