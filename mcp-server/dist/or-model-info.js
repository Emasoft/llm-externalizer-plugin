 ...
 */
export function sortedPercentiles(obj) {
 ...
 */
export function percentileAnnotation(numeric, higherIsBetter) {
    ...
    if (numeric === 50)
        return "median";
    if (numeric >= 95)
        return higherIsBetter ? `best ${fmt(100 - numeric)}%` : `worst ${fmt(100 - numeric)}%`;
    if (numeric <= 5)
 ...
 */
export function isValidOpenRouterModelId(id) {
 ...
 */
export async function fetchOpenRouterModelInfo(modelId, baseUrl, authToken, timeoutMs = MODEL_INFO_FETCH_TIMEOUT_MS) {
...
export function formatPricePerM(s) {
    if (!s)
        return "n/a";
    const n = Number(s);
    if (!isFinite(n))
...
export function qualityEmoji(level) {
    switch (level) {
        case "excellent":
        case "good":
        case "free":
 ...
 */
export function formatModelInfoMarkdown(data, modelId) {
    const lines = [];
    ...
    for (const ep of endpoints) {
        ...
        if (ep.name && ep.name !== provider)
            rows.push(["Endpoint name", ep.name]);
        if (ep.tag && ep.tag !== provider.toLowerCase())
            rows.push(["Tag", ep.tag]);
        ...
        if (ep.quantization)
            rows.push(["Quantization", ep.quantization]);
        ...
        if (ep.pricing) {
            ...
            if (p.input_cache_read)
                rows.push(["Cache-read price", priceCell(p.input_cache_read)]);
            if (p.image)
                rows.push(["Image price", priceCell(p.image)]);
            if (p.request)
                rows.push(["Request price", priceCell(p.request)]);
 ...
 */
export function formatModelInfoJson(data, _modelId) {
 ...
 */
export function formatModelInfoTable(data, modelId, colors = true) {
    const out = [];
    const endpoints = data.endpoints ?? [];
    // ── Header box ─────────────────────────────────────────────────
    ...
    out.push(paint(ANSI.bcyan, "┃ ", colors) +
    ...
    out.push(paint(ANSI.bold + ANSI.cyan, "Percentiles", colors) +
        paint(ANSI.dim, ": p50 = median (half of requests are faster) · p75 = 75th (1 in 4 slower) · p90 = 90th (1 in 10 slower) · p99 = worst 1%", colors));
    out.push(paint(ANSI.bold + ANSI.cyan, "Colors    ", colors) +
    ...
    out.push(paint(ANSI.bold + ANSI.cyan, "Pricing   ", colors) +
...
function renderEndpointTable(ep, colors) {
    ...
    if (ep.pricing) {
        ...
        if (p.input_cache_read)
            rows.push(["Cache-read price", priceCellAnsi(p.input_cache_read)]);
        if (p.image)
            rows.push(["Image price", priceCellAnsi(p.image)]);
        if (p.request)
            rows.push(["Request price", priceCellAnsi(p.request)]);
        if (p.discount !== undefined && p.discount !== 0) {
    ...
    if (typeof ep.uptime_last_5m === "number")
        rows.push(["Uptime (5m)", uptimeCell(ep.uptime_last_5m)]);
    if (typeof ep.uptime_last_30m === "number")
        rows.push(["Uptime (30m)", uptimeCell(ep.uptime_last_30m)]);
    if (typeof ep.uptime_last_1d === "number")
        rows.push(["Uptime (1d)", uptimeCell(ep.uptime_last_1d)]);
    const round = (n) => Math.round(n).toString();
    // Latency — one row per percentile. Lower is better → p99 = worst 1%.
    for (const { key, value, numeric } of sortedPercentiles(ep.latency_last_30m)) {
        const annot = percentileAnnotation(numeric, /* higherIsBetter */ false);
        const label = annot ? `Latency ${key} (${annot})` : `Latency ${key}`;
        const cell = qualityEmoji(latencyLevel(value)) +
            " " +
    ...
    for (const { key, value, numeric } of sortedPercentiles(ep.throughput_last_30m)) {
        ...
        const label = annot ? `Throughput ${key} (${annot})` : `Throughput ${key}`;
        const cell = qualityEmoji(throughputLevel(value)) +
            " " +
    ...
    if (Array.isArray(ep.supported_parameters) && ep.supported_parameters.length > 0) {
        const sorted = [...ep.supported_parameters].sort();
        const painted = sorted.map((p) => paint(ANSI.green, "✓ ", colors) + paint(ANSI.bwhite, p, colors));
        rows.push([
    ...
    const valueW = Math.max(...rows.flatMap((r) => {
        const v = r[1];
    ...
    lines.push(paint(BORDER, "│ ", colors) +
...
//# sourceMappingURL=or-model-info.js.map