 ...
 */
import { API_PRESETS, ensureSettingsExist, saveSettings, validateProfile, resolveProfile, getSettingsPath, } from "./config.js";
// ── Helpers ──────────────────────────────────────────────────────────
function die(msg) {
...
function profileFromFlags(flags) {
    ...
    if (flags.url)
        p.url = flags.url;
    if (flags.api_key)
        p.api_key = flags.api_key;
    if (flags.api_token)
        p.api_token = flags.api_token;
    if (flags.second_model)
        p.second_model = flags.second_model;
    if (flags.timeout) {
    ...
    if (flags.context_window) {
        ...
        if (!isFinite(n) || n < 0)
            die(`--context_window must be a non-negative number, got '${flags.context_window}'`);
        p.context_window = n;
    }
    if (flags.max_concurrent) {
        const n = Number(flags.max_concurrent);
        if (!isFinite(n) || n < 0)
            die(`--max_concurrent must be a non-negative number, got '${flags.max_concurrent}'`);
        p.max_concurrent = n;
    }
    if (flags.app_name)
        p.app_name = flags.app_name;
    if (flags.http_referer)
        p.http_referer = flags.http_referer;
    return p;
}
// ── Commands ─────────────────────────────────────────────────────────
function cmdList() {
...
function cmdAdd(name, args) {
    ...
    if (!flags.api)
        die(`Missing required flag: --api (${Object.keys(API_PRESETS).join(", ")})`);
    if (!flags.model)
        die("Missing required flag: --model <model-identifier>");
    const profile = profileFromFlags(flags);
    // Validate before saving
    const validation = validateProfile(name, profile);
    if (!validation.valid) {
        die(`Profile validation failed:\n${validation.errors.map((e) => `  - ${e}`).join("\n")}`);
    }
...
function cmdSelect(name) {
    ...
    const validation = validateProfile(name, profile);
    if (!validation.valid) {
        die(`Cannot select profile '${name}' — validation failed:\n` +
            validation.errors.map((e) => `  - ${e}`).join("\n"));
    }
...
function cmdEdit(name, args) {
    ...
    if (Object.keys(flags).length === 0) {
        die('No fields to edit. Use --field value pairs (e.g. --model "new-model").');
    }
    ...
    if (!validation.valid) {
        die(`Validation failed after edit:\n${validation.errors.map((e) => `  - ${e}`).join("\n")}`);
    }
...
function cmdRemove(name) {
    ...
    if (name === settings.active) {
        die(`Cannot remove the active profile '${name}'. Select a different profile first.`);
    }
    ...
    switch (subcommand) {
        ...
        case "add":
            if (!rest[0] || rest[0].startsWith("--")) {
                die("Usage: profile add <name> --mode <mode> --api <api> --model <model>");
            }
        ...
        case "mv":
            ...
            break;
        default:
            die(`Unknown profile command '${subcommand}'. Use: list, add, select, edit, remove, rename.`);
    }
}
main();
