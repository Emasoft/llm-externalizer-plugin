 ...
 */
export function parseFileGroups(paths) {
    const hasMarkers = paths.some((p) => GROUP_HEADER_RE.test(p) || GROUP_FOOTER_RE.test(p));
    if (!hasMarkers) {
    ...
    for (const entry of paths) {
        ...
        if (footerMatch) {
            if (currentGroup && footerMatch[1] !== currentGroup.id) {
                throw new Error(`Group footer '${entry}' does not match open group '${currentGroup.id}'`);
            }
            if (currentGroup && currentGroup.files.length > 0) {
                groups.push(currentGroup);
            }
 ...
 */
export function hasNamedGroups(groups) {
...
function splitBucketByBasenamePrefix(bucket, maxBytes) {
    ...
    if (byPrefix.size <= 1)
        return splitBucketBySize(bucket, maxBytes);
    const out = [];
    for (const arr of byPrefix.values()) {
        ...
        if (total <= maxBytes)
            ...
            out.push(...splitBucketBySize(arr, maxBytes));
    }
    return out;
}
/**
 ...
 */
export function autoGroupByHeuristic(paths, maxGroupBytes = AUTO_GROUP_DEFAULT_MAX_BYTES) {
    // Filter out group markers defensively — callers may pass the raw
    // input_files_paths array.
    const filtered = paths.filter((p) => typeof p === "string" &&
    ...
    for (const [, bucket] of primaryBuckets) {
        ...
        if (parts.length === 1) {
            ...
            for (let i = 0; i < fallback.length; i++) {
                const id = uniqueGroupId(sanitizeGroupId(`${rawIdBase}-p${i + 1}`), idCounts);
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            const id = uniqueGroupId(sanitizeGroupId(`${rawIdBase}${suffix}`), idCounts);
 ...
 * break the lookup.
 */
export function splitPerFileSections(content, expectedPaths) {
    const result = new Map();
    if (!content || !expectedPaths || expectedPaths.length === 0)
        return result;
