 ...
 * that the CLI reports progress (stderr progress lines) during streaming.
 *
...
import { resolveTestConfig, runCli } from './test-helpers';
 ...
 * `--quiet`. Replaces the old MCP `onprogress` callback — there is no
 * client-side notification channel any more, only this stderr stream.
 */
...
describe.skipIf(!LIVE)('chat (live)', () => {
  ...
  it('sends progress notifications during long calls', async () => {
    /** chat should write progress lines to stderr when run non-quiet */
    ...
    for (const evt of progressEvents) {
      ...
      // Restored: the CLI-harness rewrite dropped this upper bound, which left
      // the loop asserting a floor and no ceiling — a progress value of 5000
      // against total=100 would have passed. It is the bound most likely to
      // break, because progress is now parsed out of stderr text rather than
      // arriving as a typed protocol notification.
...
describe.skipIf(!LIVE)('compare_files (live)', () => {
  it('compares two files and describes differences', async () => {
    ...
    try {
      const result = await runCli(testConfig, 'compare_files', {
...
describe.skipIf(!LIVE)('batch_check (live)', () => {
  it('checks multiple files and reports per-file progress', async () => {
    /** batch_check should process each file and report batch-level progress on stderr */
    mkdirSync(TMP_DIR, { recursive: true });
