...
**Cost safety:** when `--free` is not set and the active profile is paid, run the same `scan_folder` invocation with `--estimate` first — it's a zero-cost dry run that prints the expected/ceiling cost — and proceed with the real scan only if the ceiling fits the budget.
