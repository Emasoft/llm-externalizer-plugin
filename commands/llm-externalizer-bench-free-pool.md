...
2. If the profile pins `free_models`, uses that list; otherwise falls back to the bundled `FREE_POOL_SEED` constant (the 15 seed ids the plugin ships — `poolside/laguna-m.1:free`, `deepseek/deepseek-v4-flash:free`, the two `google/gemma-4-*:free` variants, `arcee-ai/trinity-large-thinking:free`, the two `nvidia/nemotron-3-*:free` variants, `minimax/minimax-m2.5:free`, `qwen/qwen3-next-80b-a3b-instruct:free`, `openai/gpt-oss-{120b,20b}:free`, `qwen/qwen3-coder:free`, `z-ai/glm-4.5-air:free`, `meta-llama/llama-3.3-70b-instruct:free`, `nousresearch/hermes-3-llama-3.1-405b:free`; the authoritative list is `FREE_POOL_SEED` in `mcp-server/src/config.ts`).
