---
name: llm-externalizer-parallel-fixer-opus-agent
...
```
[FAILED] llm-externalizer-parallel-fixer-opus-agent — invalid input: <prompt>
```
...
```bash
BACKUP="/tmp/llm-externalizer-parallel-fixer-opus-agent.$(basename "$SOURCE_FILE").$(date +%Y%m%dT%H%M%S%z).bak"
