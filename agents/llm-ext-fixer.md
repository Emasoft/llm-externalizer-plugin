...
```
[FAILED] llm-ext-fixer — invalid input: <prompt>
```
...
```bash
BACKUP="/tmp/llm-ext-fixer.$(basename "$SOURCE_FILE").$(date +%Y%m%dT%H%M%S%z).bak"
