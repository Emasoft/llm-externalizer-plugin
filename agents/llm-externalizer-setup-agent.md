...
The snippet MUST be generated by `scripts/setup/build-snippet.py` rather than built as an inline f-string. The helper handles safe YAML quoting (model IDs with colons, embedded quotes, etc.) and rejects profile names that would create invalid YAML or collide with shell-special characters — including YAML-reserved tokens (`null`/`true`/`false`/`yes`/`no`/`on`/`off`/`~`, case-insensitive) that, emitted unquoted as a mapping key, would parse as a boolean/null. Safety-guard rejections (control chars in a value, bad/reserved profile name) exit 2; an invalid argument value exits 1.
