...
| 2026-05-24T05:14:11+0200 | in-progress → completed | SHIPPED in v9.11.0. Phase D done: added the missing CLI (`llm-externalizer cluster-synonyms`) + slash command (`/llm-externalizer-cluster-synonyms`) so it is now 3-surface compliant (was MCP-only — GAP-1 of the 3-surface audit); README + use-llm-externalizer.md updated; +10 wiring tests. Published v9.11.0 (bundled with security_scan); issue #4 closed with usage example + the 12 resolved open questions. The #41 publish blocker was cleared by the cpv.skillaudit_advisory bootstrap gate (only verified skillaudit FPs remained, advisory'd). |
