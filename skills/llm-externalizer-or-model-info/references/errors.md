...
|-|-|-|
| `only works with OpenRouter backends. Active profile is '<name>' (<mode>)` | Active profile is local (LM Studio / Ollama / vLLM / llama.cpp) | Switch to a remote profile. Use `/llm-externalizer:configure` or manually edit `~/.llm-externalizer/settings.yaml` and change the `active:` field to an OpenRouter-backed profile. |
...
| `OpenRouter returned 401` | `$OPENROUTER_API_KEY` missing or invalid | Check that the env var is set in the MCP server's process environment. Run `/llm-externalizer:discover` to see the auth status. |
...
| `could not reach OpenRouter: <err>` | Network issue, DNS failure, firewall, MCP server offline | Retry once. If still failing, run `/llm-externalizer:discover` to verify service connectivity. |
