...
| `OpenRouter returned 401` | `$OPENROUTER_API_KEY` missing or invalid | Check that the env var is set in the shell environment `llm-ext` runs in (or the plugin keychain). Run `/llm-externalizer:llm-externalizer-discover` (or `"$CLAUDE_PLUGIN_ROOT/bin/llm-ext" discover`) to see the auth status. |
...
| `could not reach OpenRouter: <err>` | Network issue, DNS failure, firewall | Retry once. If still failing, run `/llm-externalizer:llm-externalizer-discover` (or `"$CLAUDE_PLUGIN_ROOT/bin/llm-ext" discover`) to verify service connectivity. |
