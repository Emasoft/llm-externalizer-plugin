...
2. `structured_output` — JSON-schema `response_format` (the plugin's hard
   compatibility requirement)
3. `code_understanding` — find an off-by-one bug in 4 lines of Python, return
   strict JSON
4. `long_context` — ~30K-token input, return a relevant one-sentence summary
5. `output_length` — emit ≥4K tokens before stopping

This is sufficient for confirming that a model can run llm-externalizer
scans, code reviews, and reports.

Consult this skill ONLY when:

- The user explicitly asks for a deeper benchmark (e.g. "rigorous eval",
  "MMLU score", "HumanEval pass-rate before I commit to this model"), AND
- The model has already passed the wizard's 5-test check (Step 5), AND
- The user has the time / disk to run inspect-ai or lighteval against a real
  task suite (these take minutes to hours, not seconds)

The skill's output is INFORMATIONAL. The wizard's pass/fail verdict still
