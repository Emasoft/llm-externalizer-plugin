...
| `output_dir` | All content tools | string | Custom output directory for reports (default: `reports/llm-externalizer/`). |
...
- **Output location**: All responses saved to `reports/llm-externalizer/`. Tool returns ONLY the file path — never inline content.
