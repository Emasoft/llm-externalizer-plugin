...
to paste. Most setup runs never need to touch this skill.

Consult this skill when the recommender's command needs extension:

- Custom `--local-dir` placement (the recommender defaults to
  `~/models/<slug>`; a multi-user setup may want `/opt/models/<slug>` or a
  per-project tree)
- File-level filtering with `--include`/`--exclude` (e.g. download only one
  shard from a sharded safetensors repo to test, or skip `.onnx` files when
  the user wants the safetensors variant)
- Pinning to a specific `--revision` for reproducibility across sessions
- Cache management (`hf cache scan`, `hf cache delete <repo>`) — useful when
  disk pressure builds up after multiple model trials
- Auth setup (`hf auth login`) before downloading from gated repos (Llama 3
  needs a HF token tied to a Meta licence acceptance, for instance)
